...
import { updateHold } from "@/lib/hold/updatehold";
...
function SPTable({ data }) {
  ...
  const UpdateMutation = useMutation(
    (newData) => updateHold(newData), {
    onSuccess: async (data) => {
      queryClient.prefetchQuery(["ScreenplayHolds"], () =>
        getUserHold()
      );
    },
  }
  ...
  function updateHoldStatus(row, status) {
    const holdData = { screenplay_id: row.id, user_id: row.user_id, status: status }
    UpdateMutation.mutate(holdData);
        ...
        <>
          ...
          <div className="bid_status">
            <ButtonGroup aria-label="Bid Status" style={{}}>
              {(cell === "onhold") ? (
                <Button variant="light" style={{ color: "green" }}><GiSandsOfTime /></Button>
              ) : (
                <Button variant="light" style={{ color: "red" }}><MdCancel /></Button>
...
export default function HoldsData() {
  const {
    isLoading: holdLoading,
    isError,
    data: holdedUsers,
    error,
  } = useQuery(["ScreenplayHolds"], () => getUserHold());

  // console.log("holdedUsers", holdedUsers);


  return (
    <>
      <SPTable data={holdedUsers?.rows} />
    </>
  );
}

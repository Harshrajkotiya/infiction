...
export default function WriterProfileDataUpdateForm(props) {
  const {
    id: idProp,
    writerProfileData: writerProfileDataModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = writerProfileDataRecord
      ? { ...initialValues, ...writerProfileDataRecord }
      : initialValues;
    setName(cleanValues.name);
    setInfiction_username(cleanValues.infiction_username);
    setEmail(cleanValues.email);
    setDob(cleanValues.dob);
    setPhone(cleanValues.phone);
    setHighest_educatio(cleanValues.highest_educatio);
    setShort_bio(cleanValues.short_bio);
    setErrors({});
  };
  const [writerProfileDataRecord, setWriterProfileDataRecord] = React.useState(
    writerProfileDataModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(WriterProfileData, idProp)
        : writerProfileDataModelProp;
      setWriterProfileDataRecord(record);
    };
    queryData();
  }, [idProp, writerProfileDataModelProp]);
  React.useEffect(resetStateValues, [writerProfileDataRecord]);
  const validations = {
    name: [],
    infiction_username: [],
    email: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            WriterProfileData.copyOf(writerProfileDataRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "WriterProfileDataUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || writerProfileDataModelProp)}
